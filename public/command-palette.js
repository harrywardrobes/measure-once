  ...
  function renderResults(query) {
    ...
    if (q) {
      ...
      html += `<button class="cp-result-item" onclick="closeCommandPalette();location.href='/invoices?q=${encoded}'" tabindex="0">
        <span class="cp-result-icon">${invoicesIcon}</span>
        <span class="cp-result-text">
          <span class="cp-result-label">Search invoices for &ldquo;${escHtml((query || '').trim())}&rdquo;</span>
          <span class="cp-result-sub">Filter invoices by customer or invoice number</span>
        </span>
      </button>`;
    }
