...
(function () {
  ...
  async function openDesignVisitWizard(handler, ctx, existingVisit, triggerEl) {
    ...
    // Injects a single shared spinner <style> block the first time any step needs it.
    // Steps 2, 3 (and eventually Step 1) all reference the .dv-step-loading class.
    function _ensureDvSpinnerStyle() {
      if (document.getElementById('dv-spinner-style')) return;
      const styleEl = document.createElement('style');
      styleEl.id = 'dv-spinner-style';
      styleEl.textContent = '@keyframes dv-spin{to{transform:rotate(360deg)}}'
        + '.dv-step-loading{width:32px;height:32px;border:3px solid #e5e7eb;'
    ...
    async function renderStep2() {
      ...
      _ensureDvSpinnerStyle();
      reactContainer.innerHTML = '<div class="dv-step-loading" role="status" aria-label="Loading rooms step"></div>';
    ...
    async function renderStep3() {
      ...
      // React's createRoot().render() replaces this once the component mounts.
      _ensureDvSpinnerStyle();
      reactContainer.innerHTML = '<div class="dv-step-loading" role="status" aria-label="Loading review step"></div>';
