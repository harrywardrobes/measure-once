...
async function renderGoogleEmailSection() {
  ...
  el.innerHTML = `
    <div class="notes-header">
      ...
      ${isViewerOnly() ? '' : '<button class="btn-new-note" onclick="openEmailCompose()">+ Compose</button>'}
    </div>
...
function _renderRoomTabsImpl() {
  ...
  const tabs = state.allRooms.map((r, idx) => `
    <span class="room-tab-wrap ${idx === state.selectedRoomIdx ? 'room-tab-wrap-active' : ''}">
      ...
      ${canDelete && !isViewerOnly() ? `<button class="room-tab-del" title="Delete room"
  ...
  el.innerHTML = `
    <div class="flex items-center gap-1.5 flex-wrap">
      ${tabs}
      ${isViewerOnly() ? '' : '<button onclick="showAddRoomForm()" class="room-tab-add">+ Room</button>'}
    </div>
    ${addForm}
      ...
      <div class="flex items-start justify-between gap-6 flex-wrap">
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-2.5 min-w-0">
            ...
            ${isViewerOnly() ? '' : `<button class="contact-edit-btn" onclick="openContactEdit()" title="Edit contact details">
          ...
          ${(email || phone) ? `<div class="mt-3 space-y-1">
            ...
            ${phone ? `<div class="text-sm text-slate-500 flex items-center gap-1.5">
              ...
              ${state.whatsappEnabled && !isViewerOnly() ? `<button onclick="openWhatsAppModal()" title="Send WhatsApp message"
...
function renderComments() {
  ...
  el.innerHTML = `
    <div class="notes-header">
      ...
      ${isViewerOnly() ? '' : '<button class="btn-new-note" onclick="showAddComment()">+ New note</button>'}
    </div>
...
function renderTasks() {
  ...
  el.innerHTML = `
    <div class="flex items-center justify-between mb-3">
      ...
      ${isViewerOnly() ? '' : `<button onclick="toggleAddTask()" id="add-task-btn"
    ...
    return `
      <div class="qb-invoice-row">
        ...
        <div class="flex items-center gap-1" style="flex-shrink:0">
          ${_invStatusPill(statusLabel, statusKey, true)}
