  ...
  return (
    <div id="whatsapp-modal" className="wa-modal" style={{ display: 'block' }}>
      ...
      <div style={{ position: 'relative', background: 'var(--surface-card)', borderRadius: 14, width: '100%', maxWidth: 480, boxShadow: 'var(--shadow-modal)', overflow: 'hidden' }}>
        <div style={{ background: '#25D366', padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 10 }/* hex-color-ok: pre-existing raw hex */}>
          ...
          <span style={{ color: '#fff', fontSize: '1rem', fontWeight: 700 }/* hex-color-ok: pre-existing raw hex */}>Send WhatsApp</span>
          <button onClick={onClose} style={{ marginLeft: 'auto', background: 'none', border: 'none', cursor: 'pointer', color: '#fff', opacity: 0.8, fontSize: '1.25rem', lineHeight: 1, padding: '0 2px' }/* hex-color-ok: pre-existing raw hex */}>✕</button>
        </div>
        <div style={{ padding: '18px 20px' }}>
          ...
          <div style={{ display: 'flex', gap: 0, border: '1px solid var(--border-soft)', borderRadius: 8, overflow: 'hidden', marginBottom: 16 }}>
            <button id="wa-tab-template" onClick={() => setMode('template')}
              style={{ flex: 1, padding: '8px 0', fontSize: '0.82rem', fontWeight: 600, border: 'none', cursor: 'pointer', transition: 'background 0.15s', background: mode === 'template' ? '#f0f9ff' : 'var(--surface-card)' }/* hex-color-ok: pre-existing raw hex */}>
              Template
            </button>
            <button id="wa-tab-freeform" onClick={() => setMode('freeform')}
              style={{ flex: 1, padding: '8px 0', fontSize: '0.82rem', fontWeight: 600, border: 'none', borderLeft: '1px solid var(--border-soft)', cursor: 'pointer', transition: 'background 0.15s', background: mode === 'freeform' ? '#f0f9ff' : 'var(--surface-card)' }/* hex-color-ok: pre-existing raw hex */}>
              Free-form
            </button>
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 14 }}>
            ...
            <button id="wa-send-btn" onClick={send} disabled={sending}
              style={{ background: '#25D366', color: '#fff', border: 'none', borderRadius: 8, padding: '9px 22px', fontSize: '0.875rem', fontWeight: 600, cursor: 'pointer', transition: 'background 0.15s' }/* hex-color-ok: pre-existing raw hex */}>
