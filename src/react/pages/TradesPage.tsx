...
// ── Contact slot in form ───────────────────────────────────────────────────────

function ContactSlotField({
  index,
  slot,
  isFirst,
  trades,
  ...
  React.useEffect(() => {
    ...
    phoneDebounce.current = setTimeout(() => {
      setPhoneConflict(findPhoneConflict(slot.phone, trades, directory, editingId));
    }, 300);
  ...
  React.useEffect(() => {
    ...
    emailDebounce.current = setTimeout(() => {
      setEmailConflict(findEmailConflict(slot.email, trades, editingId));
    }, 300);
    return () => { if (emailDebounce.current) clearTimeout(emailDebounce.current); };
  }, [slot.email, trades, editingId]);

  return { phoneConflict, emailConflict, element: (
      ...
      <Box>
        ...
        >
          {['Phone call', 'WhatsApp', 'Email'].map((opt) => (
            <FormControlLabel key={opt} value={opt} control={<Radio size="small" />} label={<Typography variant="caption">{opt}</Typography>} />
          ))}
        </RadioGroup>
      </Box>
    </Box>
...
// ── ContactSlotField component wrapper ────────────────────────────────────────
...
function useContactSlot(
  ...
  return ContactSlotField({
    index, slot, isFirst, trades, editingId, directory,
    onEditingIdJump, onChange, onRemove,
  });
}

// ── Rendered contact slot component (proper React component) ──────────────────
