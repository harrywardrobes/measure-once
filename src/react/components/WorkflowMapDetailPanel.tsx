  ...
  return (
    <Box
      sx={{
        ...
        borderRadius: '6px',
        p: 1.5,
        bgcolor: total > 1 ? STATUS_COLORS.warningLight.bg : 'background.paper',
      }}
    >
      {total > 1 && (
      ...
      <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 0.75, mb: 1 }}>
        <BoltIcon sx={{ fontSize: 15, color: STATUS_COLORS.violet.text, mt: '1px', flexShrink: 0 }} />
        <Box>
      ...
      {interaction && (
        <Box
          sx={{
            ...
            borderRadius: '6px',
            bgcolor: interaction === 'user-input' ? STATUS_COLORS.warning.bg : STATUS_COLORS.info.bg,
            color: interaction === 'user-input' ? STATUS_COLORS.warning.text : STATUS_COLORS.info.text,
          }}
        >
    ...
    >
      ...
      >
        <Box sx={{ flex: 1, minWidth: 0 }}>
          ...
          <Typography variant="h6" sx={{ color: 'common.white', wordBreak: 'break-word', fontWeight: 700 }}>
            {node?.label}
          </Typography>
        </Box>
        <Tooltip title="Close">
                ...
                {node.hint && (
                  <Box
                    sx={{
                      ...
                      py: 1.25,
                      borderRadius: '8px',
                      bgcolor: STATUS_COLORS.infoLight.bg,
                      border: `1px solid ${STATUS_COLORS.info.bg}`,
                    }}
                  >
                    <InfoOutlinedIcon sx={{ fontSize: 16, color: 'info.main', mt: '1px', flexShrink: 0 }} />
                    <Typography variant="body2" sx={{ color: STATUS_COLORS.infoLight.text }}>
                      {node.hint as string}
                    </Typography>
                  </Box>
                )}
                  ...
                  {boundHandlers.length > 0 && (
                    <Chip
                      ...
                      sx={{
                        ...
                        fontSize: '0.65rem',
                        fontWeight: 700,
                        bgcolor: hasConflict ? STATUS_COLORS.warning.bg : STATUS_COLORS.violet.bg,
                        color: hasConflict ? STATUS_COLORS.warning.text : STATUS_COLORS.violet.text,
                        border: 'none',
                      }}
                    />
                  )}
