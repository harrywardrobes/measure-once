...
- **Production-scope observations from this scan:** the QuickBooks integration uses one shared tenant token for the whole org, and manager-class roles can currently read org-wide invoice data from that shared tenant; revoking a user from `allowed_emails` now invalidates live sessions; the public auth Turnstile flow fails closed on missing tokens and verification outages when configured; `BOOTSTRAP_ADMIN_PASSWORD` remains a standing alternate login path for any address in `ADMIN_EMAILS` whenever that secret is set; `/api/set-password` currently validates and consumes reset/set-password tokens non-atomically, so replay races must be considered in future auth reviews; the approved profile-photo flow currently accepts attacker-controlled `data:image/*` content and serves it back with the stored MIME type, so active SVG content is a standing review point; `DELETE /api/tasks/:id` currently lacks the contact-binding/object-authorization check present on task updates; generic `innerHTML` use in `public/admin.html` and the reviewed client surfaces currently relies on context-safe escaping and fixed-format identifiers; trade-company website links in `public/admin.html` and `public/trades.js` now enforce `http:`/`https:` allowlists on both server and client; WhatsApp routes in `server.js` are admin-only in the current code; member HubSpot mutation routes in `server.js` still require allowlists and object-binding checks because the app's HubSpot token has broad CRM reach.
