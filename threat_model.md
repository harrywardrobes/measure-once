...
- **Production-scope observations from this scan:** the QuickBooks integration uses one shared tenant token for the whole org; revoking a user from `allowed_emails` now invalidates live sessions; generic `innerHTML` use in `public/admin.html` was reviewed and currently relies on context-safe escaping; trade-company website links in `public/admin.html` and `public/trades.js` remain sensitive to unsafe URL schemes; the public auth surface's Turnstile helper in `auth.js` currently fails open on missing tokens and verification outages; WhatsApp routes in `server.js` must be treated as an org-wide external-integration boundary rather than a general authenticated-user feature; member HubSpot mutation routes in `server.js` need allowlists and association checks because the app's HubSpot token has broad CRM reach.
