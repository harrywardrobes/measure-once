...
- **Production-scope observations from this scan:** `auth.js` now re-validates approval status on request and invalidates sessions for removed users, so the earlier allow-list deprovisioning concern appears mitigated; `/api/set-password` now rechecks and consumes tokens transactionally, so the earlier password-link replay race appears mitigated; the customer-info public submit path now locks and submits atomically, mitigating the earlier single-row replay race; reviewed admin/client `innerHTML` sinks currently rely on static templates, fixed-format identifiers, or numeric counts rather than attacker-controlled HTML; reviewed trade-company website-link handling enforces `http:`/`https:` allowlists on both server and client; reviewed member-facing design-visit CRUD and upload-delete ownership gaps appear fixed; remaining confirmed risks are login/session reuse preserving Google OAuth state across users, non-atomic customer-info link issuance that can leave multiple valid bearer links for one contact, member-readable customer-info bearer links that enable customer impersonation, non-atomic photo-review outcome writes, unrestricted re-signing of design-visit image object keys, an admin-only QuickBooks send-rate limiter that can be bypassed with concurrent requests, and unbounded authenticated subscriptions to the HubSpot webhook SSE stream.
