...
- **Production entry points:** `server.js`, `auth.js`, `quickbooks.js`, `visits.js`, static client in `public/`.
 ...
 - **Production-scope observations from this scan:** the QuickBooks integration uses one shared tenant token for the whole org, but direct invoice read/write/PDF/send routes are currently admin-only; revoking a user from `allowed_emails` now invalidates live sessions; the public auth Turnstile flow fails closed on missing tokens and verification outages when configured; `BOOTSTRAP_ADMIN_PASSWORD` still remains a standing alternate login path for eligible `ADMIN_EMAILS` accounts whenever that secret is set, but the current cleanup logic intentionally preserves consumed `password_set_tokens` rows so the bootstrap path is not re-enabled just by routine token cleanup; `/api/set-password` now rechecks and consumes reset/set-password tokens transactionally, so the earlier replay-race concern appears mitigated; the approved profile-photo flow now restricts uploads to JPEG/PNG/WebP/GIF and serves them as attachments with `nosniff`, so the earlier active-SVG concern appears mitigated; `DELETE /api/tasks/:id` now includes the contact-binding/object-authorization check present on task updates; generic `innerHTML` use in `public/admin.html` and the reviewed client surfaces currently relies on context-safe escaping and fixed-format identifiers; trade-company website links in `public/admin.html` and `public/trades.js` enforce `http:`/`https:` allowlists on both server and client; WhatsApp routes in `server.js` are admin-only in the current code; design-visit submission and resubmission now gate client-supplied `handlerConfig.submittedLeadStatus` behind a server-side manager/admin privilege check in `runSubmitSideEffects` (members can no longer use this path to bypass the HubSpot lead-status restriction enforced by `PATCH /api/contacts/:id`); `POST /api/design-visits/:id/revision` is now admin-only (`requirePrivilege('admin')`), matching its UI affordance; member-facing design-visit CRUD still lacks object-level authorization and the design-visit upload-delete route still accepts arbitrary opaque storage keys from any authenticated member.
