...
- **Production-scope observations from this scan:** the QuickBooks integration uses one shared tenant token for the whole org; revoking a user from `allowed_emails` now invalidates live sessions; generic `innerHTML` use in `public/admin.html` and the reviewed client surfaces currently relies on context-safe escaping and fixed-format identifiers; trade-company website links in `public/admin.html` and `public/trades.js` now enforce `http:`/`https:` allowlists on both server and client; the public auth Turnstile flow fails closed on missing tokens and verification outages when configured, but still disables captcha entirely when `TURNSTILE_SECRET_KEY` is absent; `BOOTSTRAP_ADMIN_PASSWORD` remains a standing alternate login path for any address in `ADMIN_EMAILS` whenever that secret is set; WhatsApp routes in `server.js` are an org-wide external-integration boundary and are currently over-broad for ordinary member accounts; member HubSpot mutation routes in `server.js` still require allowlists and object-binding checks because the app's HubSpot token has broad CRM reach.
