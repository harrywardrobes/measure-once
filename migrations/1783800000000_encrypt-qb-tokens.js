...
// qb_tokens using AES-256-GCM (via qb-token-crypto.cjs).
//
...
const { encrypt, tryDecrypt } = require('../qb-token-crypto.cjs');

exports.shorthands = undefined;

exports.up = async (pgm) => {
  const { rows } = await pgm.db.query(
    'SELECT id, access_token, refresh_token FROM qb_tokens',
  );
      ...
      await pgm.db.query(
        `UPDATE qb_tokens
            SET access_token  = $2,
                refresh_token = $3
          WHERE id = $1`,
        [row.id, encAccessToken, encRefreshToken],
      );
    }
  }
};
