...
| `test:mount-ids` | Static check (`scripts/check-mount-id-conflicts.mjs`) that each React mount id in the `MOUNTS` table of `src/react/main.tsx` appears in at most one `public/*.html` file. A mount id duplicated across pages causes `mountKnown()` to render the wrong React island on the wrong page, making the intended island invisible. No server, no DB, no Puppeteer — reads source files directly. |
...
| `test:room-assignments-outage` | Integration guard for `GET /api/localdata/all` during a prolonged HubSpot outage. (E) asserts that room-assignment data is still returned (200, non-empty map) even after the shared contacts snapshot has aged past the stale cap — the no-cap fallback for the room-assignments view. (F) contrasts `/api/contacts-all`, which correctly returns 502 under the same conditions because the stale cap IS enforced there. (G) confirms recovery: once HubSpot comes back online the cache is refreshed and both endpoints return fresh data. Uses a mock HubSpot server and a short in-process stale-cap override; requires `DATABASE_URL_TEST` or `PRIVTEST_ALLOW_SHARED_DB=1`. |
...
**Isolated temp-database wrapper:** `scripts/with-test-db.js` creates a fresh PostgreSQL database (named `mo_testdb_<timestamp_ms>_<hex>`), sets `DATABASE_URL_TEST` for the child process, runs the target script, then drops the temp DB on exit (including SIGINT/SIGTERM). A startup pruning step runs before each new DB is created: any existing `mo_testdb_*` database older than `TEST_DB_PRUNE_TTL_MS` (default 2 hours) is dropped automatically, guarding against orphan databases left by a force-killed CI runner. Databases using the legacy naming format (`mo_testdb_<hex>`, no timestamp) are treated as infinitely old and always pruned. Used by `test:trades:ci` in `test:ci`.
