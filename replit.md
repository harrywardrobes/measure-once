...
**Isolated temp-database wrapper:** `scripts/with-test-db.js` creates a fresh PostgreSQL database (named `mo_testdb_<hex>`), sets `DATABASE_URL_TEST` for the child process, runs the target script, then drops the temp DB on exit (including SIGINT/SIGTERM). Used by `test:trades:ci` in `test:ci`.
