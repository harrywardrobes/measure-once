...
| `test:info-card-review-resend` | Puppeteer regression guard for the branching logic in `SubmissionCard` (CustomerInfoSubmissionsRail). Intercepts `/api/customer-info/submissions` to probe: **(RR-A)** submitted rows show "Review" and hide "Resend link"; **(RR-B)** pending rows show "Resend link" and hide "Review"; **(RR-C)** clicking "Review" expands the card detail without a page reload. |
