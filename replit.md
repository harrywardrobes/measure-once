...
| `test:open-leads-stale-visibility` | Tab-visibility pause for the open-leads stale badge: F1 confirms `.ls-stale-hint` is absent when `/api/open-leads` returns `X-Cache-Status: stale` while `document.hidden=true`, then appears after `visibilitychange → visible`; F2 confirms a pending clear (via `__setTestPendingOpenLeadsStale(false)`) is also deferred until visibility restores. |
