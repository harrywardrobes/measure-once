...
| `test:public-island-bootstrap` | Four-invariant static check (`scripts/check-public-island-bootstrap.mjs`) anchored to the authoritative `// public-island` annotations in the `MOUNTS` table of `src/react/main.tsx`. **Check A** (`CONN_TOAST_EXCLUDED ⊆ BOOTSTRAP_EXCLUDED`): every island excluded from `ConnectionToastProvider` must also be excluded from the `AppBootstrapProvider` auth-redirect guard. **Check B** (`(BOOTSTRAP_EXCLUDED − BOOTSTRAP_ONLY_ALLOWED) ⊆ CONN_TOAST_EXCLUDED`): every id in `BOOTSTRAP_EXCLUDED` that is not a known error page must also be in `CONN_TOAST_EXCLUDED` — catches the reverse gap where a public island is added to `BOOTSTRAP_EXCLUDED` but forgotten in `CONN_TOAST_EXCLUDED`. **Check C** (every `// public-island`-annotated MOUNTS id ∈ `CONN_TOAST_EXCLUDED`) and **Check D** (every `// public-island`-annotated MOUNTS id ∈ `BOOTSTRAP_EXCLUDED`): close the core gap that A and B cannot — a developer adds a new public mount to `MOUNTS` (correctly annotated) but forgets to add it to either or both exclusion sets; without C and D, neither A nor B would fire because both reason only about existing set contents. The `BOOTSTRAP_ONLY_ALLOWED` exemption list (`not-found-root`, `access-restricted-root`) lives at the top of the script. No server, no DB, no Puppeteer — reads source files directly. |
...
| `test:photo-storage-errors` | Regression guard for storage error sanitisation. Four probes across two server phases — phase 1 uses a constructor-throws stub, phase 2 uses an `uploadFromBytes → { ok: false }` stub: **(STO-1)** `POST /api/customer-info/:token/photos` with constructor-throws stub → HTTP 500, friendly "temporarily unavailable" copy, no raw SDK internals leaked; **(STO-2)** `POST /api/design-visits/uploads` with constructor-throws stub → HTTP 503, same guarantees; **(STO-3)** customer-info upload with `ok:false` SDK response → HTTP 500, friendly copy, no leak; **(STO-4)** design-visits upload with `ok:false` SDK response → HTTP 503, friendly copy, no leak. Uses `DATABASE_URL_TEST` or `PRIVTEST_ALLOW_SHARED_DB=1`. **In `test:ci` this runs via `test:photo-storage-errors:ci`** which automatically spins up an isolated temp database. |
