...
| `test:public-island-bootstrap` | Three-invariant static check (`scripts/check-public-island-bootstrap.mjs`) guarding the single-source-of-truth contract for public islands (task #1909). The canonical list lives in `src/react/lib/publicIslands.ts` as `PUBLIC_ISLAND_IDS`; both `CONN_TOAST_EXCLUDED` (main.tsx) and `BOOTSTRAP_EXCLUDED` (AppBootstrapContext.tsx) are derived from it at runtime. **Check A** (every `// public-island`-annotated MOUNTS id ∈ `PUBLIC_ISLAND_IDS`): catches annotation-without-set-entry — the island would still receive ConnectionToastProvider and the auth-redirect guard, silently breaking the public-facing page. **Check B** (`PUBLIC_ISLAND_IDS` ⊆ annotated MOUNTS ids): catches set-entry-without-annotation — a stale id in the Set or a MOUNTS entry removed without updating the Set. **Check C** (no `BOOTSTRAP_ONLY_IDS` id ∈ `PUBLIC_ISLAND_IDS`): error/restricted pages (`not-found-root`, `access-restricted-root`) must never appear in `PUBLIC_ISLAND_IDS`. To add a new public island: (1) add its id to `PUBLIC_ISLAND_IDS` in `src/react/lib/publicIslands.ts`; (2) annotate its MOUNTS entry in `src/react/main.tsx` with `// public-island`. No server, no DB, no Puppeteer — reads source files directly. |
