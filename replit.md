...
- Deployment build step runs `npm run build:storybook`, which writes the
  static Storybook bundle to `public/storybook/`. The Express server serves
  that directory as static assets, so the "Open Storybook →" link on the
  admin Design System tab is live on every published deploy without any
  manual build step. To refresh Storybook locally, run
  `npm run build:storybook`; the output dir is gitignored and regenerated by
  the deploy build.
