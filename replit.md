...
| `test:skipped-photo-warning` | Regression guard for the skipped-photo warning Alert added in task #1905 to `CustomerInfoSubmissionsRail.tsx`. When photos are too large to attach to the admin notification email the server stores `email_skipped_count` on the submission row; the rail must render an MUI Alert (role="alert") inside the expanded `SubmissionCard`. Three probes: **(SKP-A)** plural form (`email_skipped_count=2`) — warning Alert with "2 photos were too large…" and "they are still viewable here" is visible after expanding the card; **(SKP-B)** singular form (`email_skipped_count=1`) — Alert contains "1 photo was too large…" and "it is still viewable here"; **(SKP-C)** negative case (`email_skipped_count=0`) — no warning Alert is present after expanding the card. Boots a disposable test server and drives `/customers/:contactId` in headless Chromium; all API calls are stubbed via Puppeteer request interception so no live HubSpot or storage connection is required. Uses `DATABASE_URL_TEST` or `PRIVTEST_ALLOW_SHARED_DB=1`. |
