...
**Isolated temp-database wrapper:** `scripts/with-test-db.js` creates a fresh PostgreSQL database (named `mo_testdb_<timestamp_ms>_<hex>`), sets `DATABASE_URL_TEST` for the child process, runs the target script, then drops the temp DB on exit (including SIGINT/SIGTERM). A startup pruning step runs before each new DB is created: any existing `mo_testdb_*` database older than `TEST_DB_PRUNE_TTL_MS` (default 2 hours) is dropped automatically, guarding against orphan databases left by a force-killed CI runner. Databases using the legacy naming format (`mo_testdb_<hex>`, no timestamp) are treated as infinitely old and always pruned. Used by `test:trades:ci` and `test:bottom-nav:ci` in `test:ci`.
